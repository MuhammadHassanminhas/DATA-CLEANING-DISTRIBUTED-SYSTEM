...
def test_a_completion_after_a_reconnect_reports_on_the_new_socket():
    ...
    async def body():
        ...
        assert new.of_type("capacity"), "the completion never reached the live session"
        assert new.of_type("capacity")[0]["payload"]["task_id"] == "t1"
...
def test_a_successful_task_reports_started_then_capacity_and_leaves_no_state():
    """The "worker deletes temporary state after submission" exit
    criterion: `running` is the worker's entire local task state, and it
    must be empty once the task is done."""
    ...
    async def body():
        ...
        assert ws.types().count("capacity") == 1
