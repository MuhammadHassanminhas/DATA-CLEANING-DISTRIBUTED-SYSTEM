...
| 1.5.6 | Observability stack | **IN PROGRESS — BUILT (not yet verified live), Decisions #71–#72.** Design sub-gate approved 2026-07-27 (lean self-hosted Prometheus+Grafana+Alertmanager+Loki+Alloy, $0; alerts→Discord/Slack webhook; node_count 1→2). Built + statically verified: coordinator `/metrics` (`py_compile` OK), `infra/terraform/observability.tf` + node_count=2 (`terraform fmt`+`validate` PASS), platform chart `observability.enabled` + ServiceMonitor + PrometheusRule (`helm template` renders staging / absent production), bootstrap extended for `grafana-admin`+`alertmanager-webhook`. **Stack now LIVE on AKS + partially verified (2026-07-27, Decision #73).** `apply-observability.ps1` applied clean (2 nodes; Prometheus/Grafana/Alertmanager/Loki/Alloy all Running). PR #8 merged → CI built the `/metrics` coordinator at SHA `bc2c02e59a54e23df95daaf8bc467ad5d8a77610` → CD deployed staging. **Verified live:** staging coordinator ×2 on `bc2c02e`; **Prometheus scrapes both `up=1`** (`/metrics` over self-signed HTTPS); **Alertmanager→Discord delivery PROVEN** (built-in alerts arrived). Two live fixes: PowerShell `-target=` quoting; Alertmanager `null`-receiver (added null receiver + Watchdog route). CD gotcha fixed: staging coordinator was `0/0` (manual scale-to-0 + HPA can't lift off 0) → `kubectl scale --replicas=2` + CD re-run. **Node `az aks stop`'d after (billing $0).** **NOT DONE** — formal walkthrough of the 6 exit criteria (fleet dashboard, correlation-ID trace, fire OUR CoordinatorDown alert, auth-spike LogQL alert, retention=72h, no-secrets query) + user approval still pending. Also pending: quiet the noisy built-in kube-prometheus alerts (route only our rules to Discord). |
...
| staging | Azure AKS namespace, public Internet testing (Decision #57) | **DEPLOYED on SHA `bc2c02e` (2026-07-27) with observability** (coordinator ×2 `/metrics` scraped `up=1`, ServiceMonitor + PrometheusRule live); publicly reachable via ingress (Step 1.5.5); node `az aks stop`'d between sessions (deployments + PVCs intact) | https://dcds-staging.centralindia.cloudapp.azure.com (IP 4.240.120.113) |
| production | Azure AKS namespace, real fleet (Decision #57) | **DEPLOYED via CD approval gate to SHA `69028dee` on 2026-07-27** (was `0df7e206`); verified live in-cluster (`/health` version=`69028dee…`, `/ready` database+redis ok, 2 coordinators + dashboard + postgres + redis all Running, HPA restored to min=2). node stopped between sessions | AKS `data-cleaning-distributed-system` |
