...
| staging | Azure AKS namespace, public Internet testing (Decision #57) | **DEPLOYED on SHA `b1963f90` (2026-07-27) with observability + 1.5.6 fixes** (coordinator ×2 `/metrics` scraped `up=1`; C2 session-correlation binding + C3 `absent()` CoordinatorDown rule live; all 6 1.5.6 criteria verified — Decision #74); **Step 1.5.7 verified live 2026-07-27: coordinator HPA now min 3/max 5, namespace quota raised (limits.cpu=3), autoscale up+down demonstrated — Decision #75**; publicly reachable via ingress (Step 1.5.5); node `az aks stop`'d between sessions (deployments + PVCs intact) | https://dcds-staging.centralindia.cloudapp.azure.com (IP 4.240.120.113) |
