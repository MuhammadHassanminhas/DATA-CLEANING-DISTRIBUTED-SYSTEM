...
| 1.5.4 | GitHub Actions CD pipeline | **IN PROGRESS — BUILT, awaiting live verification + approval** (Decision #67, Option C). Design sub-gate settled 2026-07-24: hosted-runner CD deploys over the **public AKS API**, does **NOT** `az aks start/stop` (node up/down stays the user's manual credit lever — $100 student credit lasts ~12mo session-only vs ~3mo continuous). Artifacts written: `.github/workflows/cd.yml` (triggers on **CI success on `main`** via `workflow_run` — avoids the CI/CD image-push race; deploys `github.event.workflow_run.head_sha`, the same full SHA CI tags images with) + reusable `.github/workflows/_deploy-env.yml` (az login via existing 1.5.3 ARM_* secrets → `az aks get-credentials` → cluster-up guard → `helm upgrade --install --atomic` → in-cluster smoke `curl -k https://coordinator:8443/{ready,health}` + version assert → rollback-on-failure → timed record to step summary). staging auto; **production gated by a `production` GitHub Environment protection rule (required reviewer)**. Coordinator `/health` now returns `version` from a `GIT_SHA` env the Helm chart injects from `image.tag` (criterion 4). Statically verified locally: python + both workflow YAMLs parse, `helm template` renders `GIT_SHA`. **NOT yet run live** (needs `az aks start` + Azure session). **Required setup before verify:** create the `production` Environment with a required reviewer, else production auto-deploys. |
